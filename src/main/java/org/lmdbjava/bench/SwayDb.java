...
public class SwayDb {
  ...
  public void readCrc(final Reader r, final Blackhole bh) {
    r.crc.reset();
    r.db.foreach(new AbstractFunction1<scala.Tuple2<byte[], byte[]>, Object>() {
        @Override
        public Object apply(final scala.Tuple2<byte[], byte[]> entry) {
            r.crc.update(entry._1());
            r.crc.update(entry._2());
            return null;
        }
    }).materialize();
  ...
  public void readRev(final Reader r, final Blackhole bh) {
    r.db.reverse().foreach(new AbstractFunction1() {
        @Override
        public Object apply(final Object entry) {
            bh.consume(((scala.Tuple2<byte[], byte[]>) entry)._2());
            return null;
        }
    }).materialize();
  }
  ...
  public void readXxh64(final Reader r, final Blackhole bh) {
    ...
    r.db.foreach(new AbstractFunction1<scala.Tuple2<byte[], byte[]>, Object>() {
        @Override
        public Object apply(final scala.Tuple2<byte[], byte[]> entry) {
            result[0] += xx_r39().hashBytes(entry._1());
            result[0] += xx_r39().hashBytes(entry._2());
            bh.consume(entry._2());
            return null;
        }
    }).materialize();
